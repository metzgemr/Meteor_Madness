<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Meteor Madness</title>
    <link rel="stylesheet" href="../css/globe_style.css">
    <script src="../js/slider-logic.js"></script>
    <script src="../js/set-location-bg.js"></script>
</head>

<body id="body">
<script type="text/javascript">
    setBackground('../images/locations/mexico_city_day.png', '../images/locations/mexico_city_night.png');
</script>

<?php
$locationTitle = "MEXICO CITY";
$locationImageUrl = "../images/location-titles/mexico_city_title.png";
include '../components/location_header.php';
?>
<main>
    <?php
    include '../components/location_controls.php';
    ?>
</main>

<footer>

</footer>

</body>


</html>

